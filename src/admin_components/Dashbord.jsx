...
const renderActiveShape = (props) => {
    ...
    return (
        <g>
            <text x={cx} y={cy} dy={8} textAnchor="middle" fill={fill} className="text-lg font-bold">{payload.name}</text>
...
const Dashboard = () => {
    ...
    useEffect(() => {
        const fetchCompletedProjects = async () => {
            ...
            setCompletedProjectsChange(previousMonth === 0 ? 0 : ((currentMonth - previousMonth) / previousMonth) * 100);
        };
        fetchCompletedProjects();
    }, []);

    useEffect(() => {
        getOngoingProjectsCount().then(setOngoingProjectsCount).catch(() => setOngoingProjectsCount(0));
        getUpcomingProjectsCount().then(setUpcomingProjectsCount).catch(() => setUpcomingProjectsCount(0));
    }, []);

    const handleCardClick = async (status, title) => {
        const { data, error } = await supabase.from('projects').select('*, clients(name, phone)').eq('status', status);
        ...
        <div className="min-h-screen bg-gray-50 w-full text-gray-900 p-4 sm:p-6 lg:p-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
                <div onClick={() => setShowProfitPopup(true)}>
                    <Card className="cursor-pointer">
                        ...
                        <CardContent>
                            <div className="text-2xl font-bold">₹{monthlyProfit.toLocaleString('en-IN')}</div>
                            <p className={`text-xs ${profitChange >= 0 ? 'text-green-500' : 'text-red-500'}`}>
                                {profitChange >= 0 ? '+' : ''}{profitChange.toFixed(1)}% from last FY
                            </p>
                        </CardContent>
                    </Card>
                </div>
                ...
                <CardHeader>
                    ...
                    {dateFilterOpen && (
                        <div className="flex flex-col sm:flex-row gap-4 max-w-fill flex-wrap">
                            <input type="date" value={customStartDate} onChange={(e) => setCustomStartDate(e.target.value)} className="border px-3 py-2 rounded" />
                            <input type="date" value={customEndDate} onChange={(e) => setCustomEndDate(e.target.value)} className="border px-3 py-2 rounded" />
                            <button onClick={handleFilterSubmit} className="bg-green-600 text-white px-4 py-2 rounded hover:bg-green-700">Apply</button>
                            <button onClick={clearFilters} className="bg-gray-300 text-gray-800 px-4 py-2 rounded hover:bg-gray-400">Clear</button>
                        </div>
                    )}
                </CardHeader>
            ...
            <Card>
                ...
                <CardContent className="h-[400px]">
                    <ResponsiveContainer width="100%" height="100%">
                        <PieChart>
                            <Pie activeIndex={activePieIndex} activeShape={renderActiveShape} data={pieChartData} cx="50%" cy="50%" innerRadius={60} outerRadius={80} fill="#8884d8" dataKey="value" onMouseEnter={onPieEnter} style={{ outline: 'none' }}>
            ...
            {showDataView && <DataView title={dataViewTitle} data={dataViewData} onClose={() => setShowDataView(false)} />}
